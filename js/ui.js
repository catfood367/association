import { state, GLOBAL_SETTINGS_KEY, GROUP_SIZE } from './state.js';
...
export function updateModeSettingsVisibility() {
    dom.freeModeSettingsDiv.style.display = dom.modeFreeToggle.checked ? 'block' : 'none';
}
...
function _getFreeModeScoreText() {
    ...
    return `| Pontuação: ${state.score} | Level: ${state.currentGroupIndex + 1} / ${totalLevels}`;
}
...
export function showCongrats(isFsrs = false, startGameCallback) {
    if (isFsrs) {
        dom.congratsMessage.textContent = "Não há cartões novos ou para revisar no momento! Retornando ao Modo Livre.";
